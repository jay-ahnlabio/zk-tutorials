...
4.  **Verification:** 검증자는 $g^z$가 $A \cdot P^e$ 와 같은지 확인합니다. ($P(=g^x)$는 공개키)
...
### 4.2 R1CS (Rank-1 Constraint System)
코드를 수식으로 변환하기 위해 먼저 **모든 변수 값을 담은 정답지 벡터 $S$ (Witness)** 를 정의합니다.
*   $S = [1, \mathbf{a}, \mathbf{b}, \mathbf{c}, v_1, v_2, v_3]$ (여기서 $a, b$는 비밀!)

이제 각 계산 단계(Gate)를 $(S\cdot A) \times (S\cdot B) - (S\cdot C) = 0$ 형태의 **제약 조건(Constraint)(방정식)** 으로 바꿉니다. 여기서 $A, B, C$는 $S$에서 필요한 값을 꺼내오는 '집게' 역할을 합니다.

*   **Constraint 1 ($x=1$):** $a \times a - v_1 = 0$
*   **Constraint 2 ($x=2$):** $b \times b - v_2 = 0$
*   **Constraint 3 ($x=3$):** $c \times c - v_3 = 0$
*   **Constraint 4 ($x=4$):** $(v_1 + v_2) \times 1 - v_3 = 0$


### 4.3 다항식으로 변환 (QAP & Divisibility Check)
이 제약 조건들(위의 4개 방정식)을 하나의 거대한 다항식 $P(x)$로 압축합니다. 여기서 **$x$는 제약 조건의 번호(ID)** 를 의미합니다.
(하나로 묶기 위해 **"게이트 번호"를 $x$ 좌표로 사용** 하여 다항식을 만듭니다.)

*   **목표:** " $x=1, 2, 3, 4$ (게이트 번호)를 넣었을 때 결과가 모두 0이 되는 거대한 다항식 $H(x)$를 찾아라. "
*   이제 문제는 **"내가 만든 다항식이 $x=1, 2, 3, 4$에서 모두 0이 됨을 증명하라"**는 수학 문제로 바뀝니다.
*   증명자는 비밀값($S$)을 다항식 계수 속에 숨긴 채, 이 수학적 성질만을 검증자에게 보여줍니다.

#### Example
1.  **다항식 구성:** "만약 증명자가 올바른 $S$를 넣었다면, 다항식 $P(x)$는 $x=1, 2, 3, 4$일 때 결과가 모두 **0**이 되어야 한다."
2.  **Target Polynomial $T(x)$:** 0이 되어야 하는 지점들을 모은 식입니다. $T(x) = (x-1)(x-2)(x-3)(x-4)$.
3.  **검증 (Divisibility Check):**
    *   $P(x)$가 1, 2, 3, 4에서 모두 0이라면, 수학적으로 $P(x)$는 $T(x)$로 **나누어떨어져야 합니다.**
    *   즉, $P(x) = T(x) \cdot H(x)$ 를 만족하는 몫 $H(x)$가 존재해야 합니다.
    *   **결론:** 증명자가 $H(x)$를 제시할 수 있다는 것은, 모든 제약 조건을 만족하는 비밀 $S$를 알고 있다는 강력한 증거가 됩니다.
